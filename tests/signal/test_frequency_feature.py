...
from oplib.signal import mdf, mnf, vcf
...
def check_mnf_1d_array():
    ...
    output = mnf(x_1d, fs, freq_range=(10, 20))
    assert_almost_equal(output, 50 / 3)


def check_mnf_2d_arrays():
    # Get the input 2-D signal `x`.
    fs = 100
    x1 = _generate_sin_signal(1, 10, fs)
    x2 = _generate_sin_signal(2, 20, fs)
    x3 = _generate_sin_signal(1, 30, fs)
    ...
    output = mnf(x_2d.T, fs=fs, axis=0)
    assert_array_almost_equal(output, np.ones(2) * 20)
    output = mnf(x_2d.T, fs=fs, freq_range=(10, 20), axis=0)
    assert_array_almost_equal(output, np.ones(2) * 50 / 3)
    output = mnf(x_2d.T, fs=fs, freq_range=(10, 20), axis=0, keepdims=True)
    assert_array_almost_equal(output, np.ones((1, 2)) * 50 / 3)

    # Check the output when `axis` is 1.
    output = mnf(x_2d, fs=fs, axis=1)
    assert_array_almost_equal(output, np.ones(2) * 20)
    output = mnf(x_2d, fs=fs, freq_range=(10, 20), axis=1)
    assert_array_almost_equal(output, np.ones(2) * 50 / 3)
    output = mnf(x_2d, fs=fs, freq_range=(10, 20), axis=1, keepdims=True)
...
def check_mdf_1d_array():
    ...
    # Check the output when `freq_range` is not None.
    output = mdf(x_1d, fs, freq_range=(10, 15))
...
def check_mdf_2d_arrays():
    ...
    # Check the output when `axis` is 0.
    output = mdf(x_2d.T, fs=fs, axis=0)
    assert_array_almost_equal(output, np.ones(2) * 20)
    output = mdf(x_2d.T, fs=fs, freq_range=(10, 15), axis=0)
    assert_array_almost_equal(output, np.ones(2) * 10)
    output = mdf(x_2d.T, fs=fs, freq_range=(10, 15), axis=0, keepdims=True)
    assert_array_almost_equal(output, np.ones((1, 2)) * 10)

    # Check the output when `axis` is 1.
    output = mdf(x_2d, fs=fs, axis=1)
    assert_array_almost_equal(output, np.ones(2) * 20)
    output = mdf(x_2d, fs=fs, freq_range=(10, 15), axis=1)
    assert_array_almost_equal(output, np.ones(2) * 10)
    output = mdf(x_2d, fs=fs, freq_range=(10, 15), axis=1, keepdims=True)
...
def check_vcf_1d_array():
    ...
    # Check the output when `freq_range` is not None.
    output = vcf(x_1d, fs, freq_range=(10, 15))
...
def check_vcf_2d_arrays():
    ...
    # Check the output when `axis` is 0.
    output = vcf(x_2d.T, fs=fs, axis=0)
    assert_array_almost_equal(output, np.ones(2) * 50)
    output = vcf(x_2d.T, fs=fs, freq_range=(10, 15), axis=0)
    assert_array_almost_equal(output, np.zeros(2))
    output = vcf(x_2d.T, fs=fs, freq_range=(10, 15), axis=0, keepdims=True)
    assert_array_almost_equal(output, np.zeros((1, 2)))

    # Check the output when `axis` is 1.
    output = vcf(x_2d, fs=fs, axis=1)
    assert_array_almost_equal(output, np.ones(2) * 50)
    output = vcf(x_2d, fs=fs, freq_range=(10, 15), axis=1)
    assert_array_almost_equal(output, np.zeros(2))
    output = vcf(x_2d, fs=fs, freq_range=(10, 15), axis=1, keepdims=True)
