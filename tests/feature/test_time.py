...
def test_crest_factor():
    x = np.array([[4, 9, 2, 10, 0], [6, 9, 7, 12, 0]])
    assert_almost_equal(crest_factor(x), 1.6786908)
    assert_almost_equal(
        crest_factor(x, axis=0), np.array([0.3922321, 0.0, 0.9712856, 0.1810714, 0.0])
    )
    assert_almost_equal(crest_factor(x, axis=1), np.array([1.5772005, 1.52400133]))
